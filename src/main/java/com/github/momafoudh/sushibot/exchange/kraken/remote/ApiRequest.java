...
class ApiRequest {
	...
	private static final String GITHUB_NYG = "gi";
	 ...
	 * @throws IOException if the underlying {@link HttpsURLConnection} could not be
	 *                     set up or executed
	 */
	...
	/**
	 * Sets the parameters of the API method. Only supports "1-dimension" map. Nulls
	 * for keys or values are converted to the string "null".
	 *
	...
	public String setParameters(Map<String, String> parameters) throws UnsupportedEncodingException {
		...
		for (Entry<String, String> entry : parameters.entrySet()) {
			postData.append(entry.getKey()).append(EQUAL_SIGN).append(KrakenUtils.urlEncode(entry.getValue()))
					.append(AMPERSAND);
		}
